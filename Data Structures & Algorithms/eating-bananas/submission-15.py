        ...
        for i in range(50):
            if time > h:
                ...
                # else:
                    # k += 1
                time, ratio, ratio_int = measure_speed(ord_piles, k, h)
                print(f'k went up by {ceil(prev_k + delta / 2)} now: {k}, time:{time}, expected:{h}, ratio:{ratio}', ratio_int)
            elif time <= h:
                if k == 1:
                    return k
                ...
                # else:
                    # k -= 1
                time, ratio, ratio_int = measure_speed(ord_piles, k, h)
                print(f'k went down by {int(prev_k-delta / 2)} is now: {k}, time:{time}, expected:{h}, ratio:{ratio}', ratio_int)
